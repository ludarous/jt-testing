...
public interface EventRepository extends JpaRepository<Event, Long> {
    ...
    @Query(value = "select distinct event from Event event left join fetch event.workouts left join fetch event.attachedPersons",
    ...
    @Query("select event from Event event left join fetch event.workouts left join fetch event.attachedPersons where event.id =:id")
    Optional<Event> findOneWithEagerRelationships(@Param("id") Long id);

    @Query(value = "select distinct event from Event event " +
        "left join fetch event.workouts " +
