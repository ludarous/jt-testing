...
public class WorkoutResultResource {
    ...
    /**
     * POST  /workout-results : Create a new workoutResult.
     *
    ...
    public ResponseEntity<WorkoutResultDTO> createWorkoutResult(@Valid @RequestBody WorkoutResultDTO workoutResultDTO) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/workout-results/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(ENTITY_NAME, result.getId().toString()))
            .body(result);
    }

    /**
     * PUT  /workout-results : Updates an existing workoutResult.
     *
    ...
    public ResponseEntity<List<WorkoutResultDTO>> getAllWorkoutResults(Pageable pageable) {
        ...
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/workout-results");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }

    /**
     * GET  /workout-results/:id : get the "id" workoutResult.
     *
    ...
    /**
     * DELETE  /workout-results/:id : delete the "id" workoutResult.
     *
     * @param id the id of the workoutResultDTO to delete
     * @return the ResponseEntity with status 200 (OK)
     */
    @DeleteMapping("/workout-results/{id}")
    @Timed
    ...
    /**
     * SEARCH  /_search/workout-results?query=:query : search for the workoutResult corresponding
     * to the query.
     *
     ...
     */
    @GetMapping("/_search/workout-results")
    @Timed
    public ResponseEntity<List<WorkoutResultDTO>> searchWorkoutResults(@RequestParam String query, Pageable pageable) {
        ...
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/workout-results");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
