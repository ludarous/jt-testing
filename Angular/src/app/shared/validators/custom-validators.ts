...
export class CustomValidators {
  ...
  static login(c: AbstractControl) {
    if (c.value) {
      const visIdRegExp = new RegExp('^[_\'.@A-Za-z0-9-]*$');
      const valid = visIdRegExp.test(c.value);
      return valid ? null : {
        login: {
          valid: false
  ...
  static link(c: AbstractControl) {
    if (c.value) {
      const visIdRegExp = new RegExp('^(http|https)://');
      const valid = visIdRegExp.test(c.value);
      return valid ? null : {
        link: {
          valid: false
  ...
  static decimalNumberPlusMinusPointOrComma(c: AbstractControl) {
    if (c.value) {
      const visIdRegExp = new RegExp('^-?[0-9]+([\.,][0-9]{1,1000})?$');
      const valid = visIdRegExp.test(c.value);
      return valid ? null : {
        decimalNumber: {
          valid: false
  ...
  static decimalNumberPlusMinusPointOrCommaWithTwoDecimalPlaces(c: AbstractControl) {
    if (c.value) {
      const visIdRegExp = new RegExp('^-?[0-9]+([\.,][0-9]{0,2})?$');
      const valid = visIdRegExp.test(c.value);
      return valid ? null : {
