...
export class ActivitiesEditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.unitsOptions = this.units
      .map(u => ({label: this.enumTranslateService.translate(u, 'plural-1p'), value: u.ordinal}));
    ArrayUtils.insertItem(this.unitsOptions, {label: 'NONE', value: null}, 0);

    this.resultTypesOptions = this.resultTypes
      .map(rt => ({label: this.enumTranslateService.translate(rt, 'plural'), value: rt.ordinal}));
  ...
  setActivityForm(activity: IActivity, categories: Array<IActivityCategory>) {
    ...
    this.activityForm = new FormGroup({
      ...
      primaryResultValueUnit:
        new FormControl(activity.primaryResultValueUnit ? activity.primaryResultValueUnit.ordinal : null),
      secondaryResultValueUnit:
        new FormControl(activity.secondaryResultValueUnit ? activity.secondaryResultValueUnit.ordinal : null),
      minAge:
        new FormControl(activity.minAge, [CustomValidators.integerPositive, Validators.min(0), Validators.max(120)]),
      maxAge:
        new FormControl(activity.maxAge, [CustomValidators.integerPositive, Validators.min(0), Validators.max(120)]),
  ...
  saveActivity() {
    if (this.activityForm.valid) {
      ...
      saveActivity$.subscribe(
        ...
        this.activity = Activity.resolveResponse(activityResponse);
        this.setActivityForm(this.activity, this.activityCategories);
          this.toasterService.success(null, this.translateService.instant('Aktivita uložena'));
          this.router.navigate(['/pages/admin/activities/list']);
      },
        (errorResponse: HttpErrorResponse) => {
          this.toasterService.danger(null, this.translateService.instant('Aktivita nebyla uložena'));
        });
    }
  }
